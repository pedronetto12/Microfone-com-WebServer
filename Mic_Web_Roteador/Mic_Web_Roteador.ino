...
TaskHandle_t tarefa_grava;
TaskHandle_t tarefa_web;

ESP32WebServer server(80);

bool   SD_present = false; //Controls if the SD card is present or not
const char *servername = "mcserver";
...
{  
  Serial.begin(115200);
  delay(1000);
  Serial.printf("Heap: %d\n", ESP.getFreeHeap()); //verifica memoria disponivel no esp
  WiFi.softAP("ddd", "engudesc"); //Network and password for the access point genereted by ESP32
  ...
  xTaskCreatePinnedToCore(grava,"tarefa_grava",1024 * 8, NULL, 1, NULL, 0);
  delay(500);
  xTaskCreatePinnedToCore(web,"tarefa_web",1024 * 8, NULL, 2, NULL, 1);
}
...
void i2s_adc(void* arg) {
  ...
  Serial.printf("Heap before allocation: %d\n", ESP.getFreeHeap());
  ...
  Serial.printf("Heap after flash_write_buff allocation: %d\n", ESP.getFreeHeap());
  ...
  Serial.printf("Heap after i2s_read_buff allocation: %d\n", ESP.getFreeHeap());
